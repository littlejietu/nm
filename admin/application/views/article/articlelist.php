...
<div class="common">
    <span class="fl" style="line-height:30px;">分类搜索：</span><select name="class" style="float:left;margin:0 0 10px 0;" class="normalSelect fl">
        ...
        <option onclick="window.location='<?php echo base_url('index.php/articleaction/index/1/'.$v->cat_id)?>'" value="<?php echo $v->cat_id?>" <?php echo empty($classId)?'':(($classId == $v->cat_id)?'selected="selected"':'')?>><?php echo $v->cat_name?></option>
        <?php }?>
    </select>
    <table class="listTab" cellpadding="0" cellspacing="0" bordercolor="#e4e4e4" border="1">
        ...
        <tr height="30">
            ...
            <td><a href="<?php echo base_url('index.php/articleaction/addArticle/'.$value->art_id)?>">编辑</a> | <a href="javascript:;" onclick="articleDelete(this,'<?php echo  $value->art_id?>','<?php echo base_url()?>')">删除</a> </td>
        </tr>
        <?php }}?>
        <tr><td>&nbsp;</td></tr>
    </table>
