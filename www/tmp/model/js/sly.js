...
$(function($){
	...
	$(document).on('activated',function(event){
		var $section = $(".reg-scrollbox");
		var $frame = $section.find('.frame'),
			$ul = $frame.find('ul').eq(0),
			$scrollbar = $section.find('.reg-scrollbar'),
		...
		$section.find(".reg-slyWrap").each(function(i,e){
			var cont = $(this),
				frame = cont.find(".reg-sly"),
				slidee = frame.find("ul"),
				scrollbar = cont.find(".reg-scrollbar"),
