$.fn.sjld = function(){
	$('.m_zlxg p').click(function(e){
		$(this).parent().find('.m_zlxg2').slideToggle(200);
		$(this).parent().siblings('.m_zlxg').find('.m_zlxg2').slideUp(200);
		
	})
	
	$('.m_zlxg').mouseleave(function(){
		$('.m_zlxg2',this).slideUp(200);
	});
	...
	if($('#shenfen p').text()=='')
	{
		$('#shenfen p').text(sfgsmr[0].name);
		$('#sfdq_tj').val(sfgsmr[0].name);
	}
	if($('#chengshi p').text()=='')
	{
		$('#chengshi p').text(csgsmr[0].name);
		$('#csdq_tj').val(csgsmr[0].name);
	}
	if($('#quyu p').text()=='')
	{
		$('#quyu p').text(qygsmr[0]);
		$('#qydq_tj').val(qygsmr[0]);
	}
	//默认城市
	...
	for(c=0;c<qygsmr.length;c++){
		var quyumcmr = qygsmr[c].name;
		var quyunrmr = "<li>"+quyumcmr+"</li>";
		$('#quyu').find('ul').append(quyunrmr);
	}
...
/*---------------------------------------------------------------------*/
	...
	$('#shenfen ul li').click(function(){
		var dqsf = $(this).text();
		$('#shenfen').find('p').text(dqsf);
		$('#shenfen').find('p').attr('title',dqsf);
		if($('#baseUrl').length>0)
		{
			changeExpressCost('',dqsf,1,$('#baseUrl').val());
		}
		...
		var quyugsdqmr = provinceList[sfnum].cityList[0].areaList;
		$('#quyu').find('ul').text('');
		for(j=0;j<quyugsdqmr.length;j++){
			var quyumc = quyugsdqmr[j];
			var quyunr = "<li>"+quyumc+"</li>";
			$('#quyu').find('ul').append(quyunr);
		}	
		
		

		$('#chengshi p').text(csgs[0].name);
		$('#quyu p').text(csgs2[0]);
		$('#sfdq_num').val(sfnum);
/*---------------------------------------------------------------------*/
        $('#chengshi ul li').click(function(){
			$(this).parent().parent().slideUp(200);
			var dqcs = $(this).text();
	   	 ...
	   	 var quyugs = provinceList[dqsf_num].cityList[csnum].areaList;
			$('#quyu').find('ul').text('');
			for(j=0;j<quyugs.length;j++){
				var quyumc = quyugs[j];
				var quyunr = "<li>"+quyumc+"</li>";
				$('#quyu').find('ul').append(quyunr);
	   	 }	
	    
			$('#quyu p').text(quyugs[0]);
			$('#csdq_num').val(csnum);
...
$('.m_zlxg').click(function(){
	$('#sfdq_tj').val($('#shenfen p').text());
	$('#csdq_tj').val($('#chengshi p').text());
	$('#qydq_tj').val($('#quyu p').text());
	})//表单传值获取
