...
class Register extends CI_Controller {
    ...
    public function index()
    {
    	...
    	$this->view->_template->js_path  = $this->yun_url.'/assets/src/lib/';
        if ($this->loginUser['id'])
        {
            redirect('/m/index');
        }
        $page = 'user/reg_v4';
        $result['forword_url'] = $_SERVER['HTTP_REFERER'];
        $result['seo']  = get_seo('register');
        $this->load->library('des');
        
        $u = $this->input->get('u');
        if($u){
        	$this->view->assign('is_invite',$u);
        }
    ...
    public function xm()
    {
        ...
        $result['user_type'] = 1;
        $this->view->assign('result', $result);
        $this->view->display('user/reg_zjxm');
    }
